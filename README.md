...
<a href="VERSION"><img src="https://img.shields.io/badge/version-1.0.0--beta.2-blue?style=flat-square" alt="version"></a>
...
Releases must run from a major branch named `0.x`, `1.x`, `2.x`, and so on. The branch determines the release major; `0.x` is supported for SemVer's initial-development lifecycle. Other branch names and detached HEAD states abort the workflow. Maintainer retrieves every published GitHub release through the authenticated GitHub CLI, ignores drafts and unsupported tags, then selects the highest valid version for the branch major. Before analyzing changes, it verifies that the selected GitHub release tag resolves to a commit in the local clone. A missing local tag is fetched explicitly from `origin`, allowing shallow clones and clones created with `--no-tags` to release safely; an existing local tag does not require network access. If `0.x` has no valid GitHub release, its initial choices follow SemVer's recommendation: `0.1.0`, `0.1.0-alpha.1`, and `0.1.0-beta.1`. Other majors start at `MAJOR.0.0` with equivalent alpha and beta choices.
...
Git remains the source of truth for changelog commit hashes. The changelog agent may enrich only the abbreviated hashes returned by `git log`; entries with empty or unknown hashes are ignored because they commonly describe temporary version, badge, changelog, or build changes created before the release commit exists. Every real commit omitted by the agent receives a deterministic fallback entry derived from its Conventional Commit subject, so a malformed AI hash cannot abort the release or silently remove a committed change. A release with no commits since its selected base release is rejected before asking AI to invent content.
